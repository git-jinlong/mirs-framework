...
public class ArrayUtil {
    ...
    public static Object[] defaultIfEmpty(Object[] array, Class<?> defaultComponentType) {
        return ((array == null) || (array.length == 0)) ? (Object[]) Array.newInstance(ClassUtil.getNonPrimitiveType(defaultComponentType), 0)
                : array;
    }
    ...
    public static int[] toIntArray(String str, String splitPattern) {
        String[] strArray = str.split(splitPattern);
    ...
    public static <T> T[] topN(T[] data, Comparator<T> c) {
        int len = data.length;
        int firstNonLeafIndex = (len - 2) / 2;
        for (int i = firstNonLeafIndex; i >= 0; i--) {
            maxHeapify(data, c, len, i);
        }
        for (int j = len - 1; j > 0; j--) {
            swap(data, j, 0);
            maxHeapify(data, c, j, 0);
        }
        return data;
    }

    private static <T> void maxHeapify(T[] a, Comparator<T> c, int len, int subTreeNodeMax) {
        int left = subTreeNodeMax * 2 + 1;
        int right = subTreeNodeMax * 2 + 2;
        int maxIndex = subTreeNodeMax;
        if (left < len && c.compare(a[left], a[maxIndex]) > 0) {
            maxIndex = left;
        }
        if (right < len && c.compare(a[right], a[maxIndex]) > 0) {
            maxIndex = right;
        }
        if (maxIndex != subTreeNodeMax) {
            swap(a, maxIndex, subTreeNodeMax);
            maxHeapify(a, c, len, maxIndex);
        }
    }
